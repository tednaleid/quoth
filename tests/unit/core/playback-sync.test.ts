...
const knees = makeHorizonKnees(10);
...
describe('makeHorizonKnees', () => {
  it('scales all knees proportionally from horizonSeconds', () => {
    const k = makeHorizonKnees(10);
    // Historical defaults reproduced at horizonSeconds=10.
    expect(k.futureFull).toBeCloseTo(1.5);
    expect(k.futureMid).toBeCloseTo(3);
    expect(k.futureFar).toBe(10);
    expect(k.pastFull).toBeCloseTo(0.75);
    expect(k.pastMid).toBeCloseTo(1.5);
    expect(k.pastFar).toBe(5);
  });

  it('maintains the 2x past:future decay ratio at any horizon', () => {
    const k = makeHorizonKnees(20);
    expect(k.pastFar).toBeCloseTo(k.futureFar / 2);
    expect(k.pastMid).toBeCloseTo(k.futureMid / 2);
    expect(k.pastFull).toBeCloseTo(k.futureFull / 2);
  });

  it('scales linearly (doubling horizonSeconds doubles every knee)', () => {
    const a = makeHorizonKnees(5);
    const b = makeHorizonKnees(10);
    expect(b.futureFar).toBeCloseTo(a.futureFar * 2);
    expect(b.futureMid).toBeCloseTo(a.futureMid * 2);
    expect(b.futureFull).toBeCloseTo(a.futureFull * 2);
    expect(b.pastFar).toBeCloseTo(a.pastFar * 2);
    expect(b.pastMid).toBeCloseTo(a.pastMid * 2);
    expect(b.pastFull).toBeCloseTo(a.pastFull * 2);
  });
});
...
describe('horizonIntensity', () => {
  ...
  describe('future (approaching) words', () => {
    it('is 1.0 at the exact start time', () => {
      expect(horizonIntensity(wordAt(10_000), 10_000, knees)).toBe(1.0);
    });

    it('is 1.0 within the full-intensity window (0 < d < 1.5s)', () => {
      expect(horizonIntensity(wordAt(10_500), 10_000, knees)).toBe(1.0);
      expect(horizonIntensity(wordAt(11_499), 10_000, knees)).toBe(1.0);
    });

    it('is 1.0 at the 1.5s knee (ramp begin)', () => {
      expect(horizonIntensity(wordAt(11_500), 10_000, knees)).toBe(1.0);
    });

    it('is 0.75 at the 1.5s-3s midpoint (d=2.25s)', () => {
      expect(horizonIntensity(wordAt(12_250), 10_000, knees)).toBeCloseTo(0.75);
    });

    it('is 0.5 at the 3s knee', () => {
      expect(horizonIntensity(wordAt(13_000), 10_000, knees)).toBeCloseTo(0.5);
    });

    it('is 0.25 at the 3s-10s midpoint (d=6.5s)', () => {
      expect(horizonIntensity(wordAt(16_500), 10_000, knees)).toBeCloseTo(0.25);
    });

    it('approaches 0 just before the 10s knee', () => {
      expect(horizonIntensity(wordAt(19_999), 10_000, knees)).toBeCloseTo(0.0, 3);
    });

    it('is 0 at the 10s knee', () => {
      expect(horizonIntensity(wordAt(20_000), 10_000, knees)).toBe(0);
    });

    it('is 0 well beyond the 10s future window', () => {
      expect(horizonIntensity(wordAt(30_000), 10_000, knees)).toBe(0);
    });
  });

  describe('past (decaying) words', () => {
    it('is 1.0 within the 0.75s past-full window', () => {
      expect(horizonIntensity(wordAt(9_500), 10_000, knees)).toBe(1.0);
      expect(horizonIntensity(wordAt(9_251), 10_000, knees)).toBe(1.0);
    });

    it('is 1.0 at the 0.75s knee (ramp begin)', () => {
      expect(horizonIntensity(wordAt(9_250), 10_000, knees)).toBe(1.0);
    });

    it('is 0.75 at the 0.75s-1.5s midpoint (d=1.125s)', () => {
      expect(horizonIntensity(wordAt(8_875), 10_000, knees)).toBeCloseTo(0.75);
    });

    it('is 0.5 at the 1.5s knee', () => {
      expect(horizonIntensity(wordAt(8_500), 10_000, knees)).toBeCloseTo(0.5);
    });

    it('is 0.25 at the 1.5s-5s midpoint (d=3.25s)', () => {
      expect(horizonIntensity(wordAt(6_750), 10_000, knees)).toBeCloseTo(0.25);
    });

    it('is 0 at the 5s knee', () => {
      expect(horizonIntensity(wordAt(5_000), 10_000, knees)).toBe(0);
    });

    it('is 0 well beyond the 5s past window', () => {
      expect(horizonIntensity(wordAt(0), 10_000, knees)).toBe(0);
    });
  });
  ...
  describe('asymmetry (past decays 2x faster than future ramps)', () => {
    ...
    it('50% intensity at +3s future equals 50% at -1.5s past', () => {
      const futureMid = horizonIntensity(wordAt(13_000), 10_000, knees);
      const pastMid = horizonIntensity(wordAt(8_500), 10_000, knees);
...
describe('findHorizonWindow', () => {
  ...
  it('returns [-1, -1] for empty words', () => {
    expect(findHorizonWindow([], 10_000, knees)).toEqual([-1, -1]);
  });
  ...
  it('returns [-1, -1] when nowMs is far before any word', () => {
    ...
    expect(findHorizonWindow(spacedWords, -20_000, knees)).toEqual([-1, -1]);
  });
  ...
  it('returns [-1, -1] when nowMs is far past the last word', () => {
    ...
    expect(findHorizonWindow(spacedWords, 200_000, knees)).toEqual([-1, -1]);
  });

  it('includes words within [nowMs - 5000, nowMs + 10000]', () => {
    // nowMs=30_000: window is word.start in [25_000, 40_000] → indices 25..40
    const [start, end] = findHorizonWindow(spacedWords, 30_000, knees);
  ...
  it('clips to the end of the word array', () => {
    ...
    const [start, end] = findHorizonWindow(spacedWords, 58_000, knees);
